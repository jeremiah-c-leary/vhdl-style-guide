...
class rule_500(token_case_with_prefix_suffix):
    ...
    **Fix**
    ...
    def generate_regexp_exceptions(self):
        self.regexp_exceptions = [re.compile(sException, re.IGNORECASE) for sException in self.exceptions]

    def exception_found(self, sToken):
        return any(oRegex.match(sToken) for oRegex in self.regexp_exceptions)
    ...
    def _get_tokens_of_interest(self, oFile):
        ...
        for dDict in lAllDicts:
            ...
            for iName in dDict["names"]:
                ...
                if sName in lLocalDeclarationIdentifiers:
                    continue
                if self.exception_found(sName):
                    continue
