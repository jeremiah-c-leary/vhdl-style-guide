...
class rule_002(block_rule.Rule):
    ...
    **Fix**
    ...
    def analyze_comments(self, oToi):
        ...
        iStyleIndex = self.select_style_index(oToi)
        ...
        for oToken in lTokens:
            ...
            if middle_comment(iComment, iComments, oToken):
                analyze_middle_comment(self, oToken, oToi, iLine, dStyle)
...
def analyze_middle_comment(self, oToken, oToi, iLine, dStyle):
    self.set_token_indent(oToken)

    if dStyle["comment_left"] is None:
        return None

    sHeader = self.build_comment(oToken, dStyle)
