...
class Rule(structure.Rule):
    ...
    def fix(self, oFile, dFixOnly=None):
        ...
        if self.fixable:
            ...
            # rule.Rule.fix() skips analyze() when fixable is False, but
            # block_comment_002 relies on analyze() running so its
            # set_token_indent side effect is applied during the fix pass.
            self.analyze(oFile)
