...
def get_tokens_bounded_by_tokens_unless_token_is_between_them(oStart, oEnd, oToken, lAllObjects, oTokenMap):
    lReturn = []
    ...
    for iStart, iEnd in zip(lStart, lEnd):
        bTokenIsBetween = False
        for iIndex in lIndexes:
            if iStart < iIndex and iIndex < iEnd:
                bTokenIsBetween = True
                continue
        if not bTokenIsBetween:
