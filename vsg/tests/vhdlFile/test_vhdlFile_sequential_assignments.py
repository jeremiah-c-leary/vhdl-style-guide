...
oFile = vhdlFile.vhdlFile(os.path.join(os.path.dirname(__file__),'..','rule_sequential','sequential_test_input.vhd'))
...
class testVhdlFileSequentialAssignments(unittest.TestCase):
    ...
    def test_insideSequential_assignment(self):
        lExpected = [13,14,15,20,21,22,26,27,28,33,34,38,39,40,53,54,55,56,57,58,63,65,66,71,73,74,75,80,81,83,88,89,90]
        # Generic actual list
        lActual = []
        for iIndex, oLine in enumerate(oFile.lines):
    ...
    def test_isSequentialEnd_assignment(self):
        lExpected = [13,14,15,20,21,22,26,27,28,33,34,38,39,40,55,56,58,63,65,66,71,74,75,80,81,83,88,89,90]
        # Generic actual list
        lActual = []
        for iIndex, oLine in enumerate(oFile.lines):
