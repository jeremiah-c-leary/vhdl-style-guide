...
class testVsg(unittest.TestCase):
    ...
    def test_single_configuration_w_rule_disable(self):
        ...
        lActual = subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_3.json','--output_format','syntastic','-f','tests/vsg/entity1.vhd'])
    ...
    def test_multiple_configuration_w_rule_disable(self):
        ...
        try:
            subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_3.json','tests/vsg/config_4.json','--output_format','syntastic','-f','tests/vsg/entity1.vhd'])
        except subprocess.CalledProcessError as e:
            lActual = str(e.output.decode('utf-8')).split('\n')
            iExitStatus = e.returncode
    ...
    def test_reverse_multiple_configuration_w_rule_disable(self):
        ...
        lActual = subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_4.json','tests/vsg/config_3.json','--output_format','syntastic','-f','tests/vsg/entity1.vhd'])
        lActual = str(lActual.decode('utf-8')).split('\n')
        self.assertEqual(lActual, lExpected)
    ...
    def test_invalid_configuration(self):
        ...
        try:
            lActual = subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_error.json','--output_format','syntastic','-f','tests/vsg/entity1.vhd','--junit',config_error_file])
        except subprocess.CalledProcessError as e:
            lActual = str(e.output.decode('utf-8')).split('\n')
            iExitStatus = e.returncode
    ...
    def test_local_rules(self):
        ...
        try:
            subprocess.check_output(['bin/vsg', '--style', 'jcl', '-f', 'tests/vsg/entity_architecture.vhd', '-of', 'syntastic', '-lr', 'tests/vsg/local_rules'])
            iExitStatus = 0
        except subprocess.CalledProcessError as e:
            lActual = str(e.output.decode('utf-8')).split('\n')
            iExitStatus = e.returncode
    ...
    def test_invalid_local_rule_directory(self):
        lExpected = ['ERROR: encountered FileNotFoundError, No such file or directory tests/vsg/invalid_local_rule_directory when trying to open local rules file.', '']

        try:
            lActual = subprocess.check_output(['bin/vsg', '-f', 'tests/vsg/entity_architecture.vhd', '-of', 'syntastic', '-lr', 'tests/vsg/invalid_local_rule_directory'])
        except subprocess.CalledProcessError as e:
            lActual = str(e.output.decode('utf-8')).split('\n')
            iExitStatus = e.returncode
...
#        print(lExpected)
    ...
    def test_single_yaml_configuration_w_rule_disable(self):
        ...
        lActual = subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_3.yaml','--output_format','syntastic','-f','tests/vsg/entity1.vhd'])
        lActual = str(lActual.decode('utf-8')).split('\n')
        self.assertEqual(lActual, lExpected)
    ...
    def test_multiple_yaml_configuration_w_rule_disable(self):
        ...
        try:
            subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_3.yaml','tests/vsg/config_4.yaml','--output_format','syntastic','-f','tests/vsg/entity1.vhd'])
        except subprocess.CalledProcessError as e:
            lActual = str(e.output.decode('utf-8')).split('\n')
            iExitStatus = e.returncode
    ...
    def test_reverse_yaml_multiple_configuration_w_rule_disable(self):
        ...
        lActual = subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_4.yaml','tests/vsg/config_3.yaml','--output_format','syntastic','-f','tests/vsg/entity1.vhd'])
        lActual = str(lActual.decode('utf-8')).split('\n')
        self.assertEqual(lActual, lExpected)
    ...
    def test_summary_output_format_error_with_local_rules(self):
        ...
        try:
            subprocess.check_output(['bin/vsg', '-f', 'tests/vsg/entity_architecture.vhd', '-of', 'summary', '-lr', 'tests/vsg/local_rules'], stderr=subprocess.PIPE)
            iExitStatus = 0
        except subprocess.CalledProcessError as e:
            lActualStdOut = str(e.output.decode('utf-8')).split('\n')
            lActualStdErr = str(e.stderr.decode('utf-8')).split('\n')
    ...
    def test_summary_output_format_ok(self):
        ...
        lActual = subprocess.check_output(
                ['bin/vsg', '-f', 'tests/vsg/entity_architecture.fixed.vhd', '-of', 'summary'],
    ...
    def test_configuration_with_file_rules_and_no_file_list_entity2(self):
        ...
        try:
            subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_file_rules.yaml', '-f', 'tests/vsg/entity2.vhd', '--output_format','syntastic'])
        except subprocess.CalledProcessError as e:
            lActual = str(e.output.decode('utf-8')).split('\n')
            iExitStatus = e.returncode
    ...
    def test_configuration_with_file_rules_and_no_file_list_entity1(self):
        ...
        try:
            subprocess.check_output(['bin/vsg','--configuration','tests/vsg/config_file_rules.yaml', '-f', 'tests/vsg/entity1.vhd', '--output_format','syntastic'])
        except subprocess.CalledProcessError as e:
            lActual = str(e.output.decode('utf-8')).split('\n')
            iExitStatus = e.returncode
