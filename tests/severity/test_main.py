...
class test_severity_using_main(unittest.TestCase):
    ...
    def test_entity_with_configuration_and_fixed(self):
        lActual = subprocess.check_output(
                ['bin/vsg', '-f', self._sEntityFileName, '-c', sConfigFile, '--fix']
                ).decode('utf-8').split('\n')
    ...
    def test_architecture_with_configuration_and_fixed(self):
        lActual = subprocess.check_output(
                ['bin/vsg','-f', self._sArchitectureFileName, '-c', sConfigFile, '--fix']
                ).decode('utf-8').split('\n')
    ...
    def test_both_with_configuration(self):
        ...
        self.assertEqual(iExitStatus, 1)
        self.assertEqual(utils.replace_token(utils.replace_token(utils.replace_total_count(lActual), self._sEntityFileName, sEntityFileName), self._sArchitectureFileName, sArchitectureFileName), lExpected1 + lExpected2)

    def test_both_with_configuration_and_fixed(self):
        lActual = subprocess.check_output(
                ['bin/vsg', '-f', self._sEntityFileName, self._sArchitectureFileName, '-c', sConfigFile, '--fix']
                ).decode('utf-8').split('\n')
