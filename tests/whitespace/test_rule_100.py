...
class test_rule(unittest.TestCase):
    ...
    def test_rule_100(self):
        oRule = whitespace.rule_100()
        self.assertTrue(oRule)
        self.assertEqual(oRule.name, "whitespace")
        self.assertEqual(oRule.identifier, "100")

        lExpected = []
        lExpected.extend(range(25, 37))
        lExpected.extend(range(42, 54))
        lExpected.extend(range(59, 71))
        lExpected.extend(range(76, 88))
        lExpected.extend(range(93, 105))
        lExpected.extend(range(110, 122))
